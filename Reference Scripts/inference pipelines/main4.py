...
class YoloTrainerApp(ctk.CTk):
    ...
    def _open_results_folder(self):
        """Open the training results folder based on UI input."""
        project = self.project_name_entry.get().strip() or DEFAULT_PROJECT_NAME
        exp = self.exp_name_entry.get().strip() or "exp"
        path_to_open = os.path.abspath(os.path.join(project, exp))

        if os.path.isdir(path_to_open):
            logging.info(f"[INFO] Opening training results folder: {path_to_open}")
            self._open_folder_native(path_to_open)
        else:
            logging.warning(f"[WARNING] Training folder not found at: {path_to_open}")
            messagebox.showwarning(
                "Results Folder Not Found",
                f"Folder not found:\n{path_to_open}\n\nMake sure training has started and this folder exists."
        )
