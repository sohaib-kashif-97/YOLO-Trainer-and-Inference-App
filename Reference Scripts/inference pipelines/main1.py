...
class YoloTrainerApp(ctk.CTk):
    ...
    def _run_live_video_inference(self, model_path, source_path, conf, iou, device):
        """Run YOLO inference on video frame-by-frame and update GUI live."""
        try:
            ...
            frame_num = 0
            while cap.isOpened() and not self.infer_stop_event.is_set():
                ret, frame = cap.read()
                if not ret:
                    break
                ...
                self.after(0, self._update_infer_output_label, photo)
                frame_num += 1
    ...
    def _update_infer_output_label(self, photo):
    ...
    def _start_inference(self):
        ...
        if self.infer_source_type == 'video':
            ...
            self.inference_thread = threading.Thread(
                ...
                args=(weights, self.infer_source_path, conf, iou, device),
