...
class YoloTrainerApp(ctk.CTk):
    ...
    def _create_section_header(self, parent, text):
        # Modern, bold, larger header with more padding
        header = ctk.CTkLabel(parent, text=text, font=ctk.CTkFont(size=20, weight="bold"), anchor="w", pady=12)
        return header

    # --- Training Tab UI Creation (Modified to accept parent) ---
    def _create_config_panel(self, parent_frame): # Added parent_frame argument
        config_outer_frame = ctk.CTkFrame(parent_frame, width=400, corner_radius=16)  # More width, more rounded
        config_outer_frame.grid(row=0, column=0, padx=(0, 20), pady=0, sticky="nsew") # More right padding
        ...
        config_header.pack(fill="x", padx=PAD_X*2, pady=PAD_Y_SECTION)

        config_frame = ctk.CTkFrame(config_outer_frame, fg_color="#23272F", corner_radius=12)  # Subtle background, rounded
        config_frame.pack(fill="both", expand=True, padx=PAD_X, pady=PAD_Y)
    ...
    def _create_status_panel(self, parent_frame): # Added parent_frame argument
        status_frame = ctk.CTkFrame(parent_frame, corner_radius=16)
        ...
        # --- 1. Control Bar ---
        control_bar_frame = ctk.CTkFrame(status_frame, fg_color="#23272F", corner_radius=12)
        control_bar_frame.grid(row=current_row, column=0, padx=PAD_X*2, pady=(PAD_Y*2, PAD_Y), sticky="ew")
        control_bar_frame.grid_columnconfigure(3, weight=1) # Spacer col
        self.start_button = ctk.CTkButton(control_bar_frame, text="Start Training", image=self.icon_play, compound="left", width=160, height=38, font=ctk.CTkFont(size=14, weight="bold"), corner_radius=12)
        self.start_button.grid(row=0, column=0, padx=(0, PAD_Y), pady=PAD_Y)
        self.stop_button = ctk.CTkButton(control_bar_frame, text="Stop Training", image=self.icon_stop, compound="left", width=160, height=38, font=ctk.CTkFont(size=14, weight="bold"), corner_radius=12, state="disabled", fg_color="#D32F2F", hover_color="#C62828")
        self.stop_button.grid(row=0, column=1, padx=PAD_Y, pady=PAD_Y)
        self.results_button = ctk.CTkButton(control_bar_frame, text="Open Results", image=self.icon_folder, compound="left", width=160, height=38, font=ctk.CTkFont(size=14, weight="bold"), corner_radius=12, state="disabled")
    ...
    def _create_infer_config_panel(self, parent_frame):
        ...
        config_outer_frame = ctk.CTkFrame(parent_frame, width=400, corner_radius=16)
        config_outer_frame.grid(row=0, column=0, padx=(0, 20), pady=0, sticky="nsew")
        config_outer_frame.grid_rowconfigure(1, weight=1) # Allow content frame to expand
        config_outer_frame.grid_propagate(False)
        ...
        config_frame = ctk.CTkFrame(config_outer_frame, fg_color="#23272F", corner_radius=12)
        config_frame.pack(fill="both", expand=True, padx=PAD_X, pady=PAD_Y)
        config_frame.grid_columnconfigure(1, weight=1)
    ...
    def _create_infer_output_panel(self, parent_frame):
        ...
        output_frame = ctk.CTkFrame(parent_frame, corner_radius=16)
...
if __name__ == "__main__":
    ...
    # --- MODERN THEME SETUP ---
    try:
        ctk.set_default_color_theme("modern_theme.json")
    except Exception as e:
        print(f"[WARNING] Could not load modern theme: {e}")
